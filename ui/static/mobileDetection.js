// Obtain the user agent string from the browser
const userAgentString = navigator.userAgent || navigator.vendor || window.opera;

// A function to test if the user's device is a mobile device (based on http://detectmobilebrowsers.com/)
function isMobileDevice(userAgent) {
  // A pattern to match against the user agent string; tests for a wide variety of mobile user agent strings
  const mobileDevicePattern = new RegExp(
    "(android|bb\\d+|meego).+mobile|avantgo|bada\\/|blackberry|blazer|compal|elaine|fennec|hiptop|" +
      "iemobile|ip(hone|od)|iris|kindle|lge |maemo|midp|mmp|mobile.+firefox|netfront|" +
  ...
  // A pattern to match against mobile browsers which can be detected via the first four characters of the user agent string
  const shortMobileDevicePattern = new RegExp(
    "1207|6310|6590|3gso|4thp|50[1-6]i|770s|802s|a wa|abac|ac(er|oo|s\\-)|" +
      ...
      "cmd\\-|co(mp|nd)|craw|da(it|ll|ng)|dbte|dc\\-s|devi|dica|dmob|do(c|p)o|" +
      "ds(12|\\-d)|el(49|ai)|em(l2|ul)|er(ic|k0)|esl8|ez([4-7]0|os|wa|ze)|" +
      "fetc|fly(\\-|_)|g1 u|g560|gene|gf\\-5|g\\-mo|go(\\.w|od)|gr(ad|un)|" +
      "haie|hcit|hd\\-(m|p|t)|hei\\-|hi(pt|ta)|hp( i|ip)|hs\\-c|" +
      "ht(c(\\-| |_|a|g|p|s|t)|tp)|hu(aw|tc)|i\\-(20|go|ma)|i230|" +
      "iac( |\\-|\\/)|ibro|idea|ig01|ikom|im1k|inno|ipaq|iris|ja(t|v)a|" +
      "jbro|jemu|jigs|kddi|keji|kgt( |\\/)|klon|kpt |kwc\\-|kyo(c|k)|" +
      "le(no|xi)|lg( g|\\/(k|l|u)|50|54|\\-[a-w])|libw|lynx|m1\\-w|m3ga|" +
      "m50\\/|ma(te|ui|xo)|mc(01|21|ca)|m\\-cr|me(rc|ri)|mi(o8|oa|ts)|mmef|" +
      ...
      "qa\\-a|qc(07|12|21|32|60|\\-[2-7]|i\\-)|qtek|r380|r600|raks|rim9|" +
      ...
      "to(pl|sh)|ts(70|m\\-|m3|m5)|tx\\-9|up(\\.b|g1|si)|utst|v400|v750|" +
  ...
  // Test the user agent against the patterns for mobile devices
  return (
    mobileDevicePattern.test(userAgent) ||
    shortMobileDevicePattern.test(userAgent.substr(0, 4))
  );
}

function redirectMobileUsers(userAgentString) {
